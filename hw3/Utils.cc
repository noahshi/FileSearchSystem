...
void CRC32::FoldBytesIntoCRC(const uint8_t *next_bytes, size_t num_bytes) {
  Verify333(finalized_ != true);

  size_t batches = num_bytes / 8;
  uint32_t crc = crc_state_;

  for (size_t i = 0; i < batches; ++i) {
    uint64_t bytes = *(reinterpret_cast<const int64_t*>(&next_bytes[i*8]));
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ (bytes & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 8) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 16) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 24) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 32) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 40) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 48) & 0xFF)];
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ ((bytes >> 56) & 0xFF)];
  }

  for (size_t i = batches * 8; i < num_bytes; ++i) {
    crc = (crc >> 8) ^ table_[(crc & 0xFF) ^ next_bytes[i]];
  }
