...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  while (left_to_read > 0) {
    int bytes_read = fread(&buf, 1, kBufSize, f);
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int record_size = WriteHTBucketRecord(f,
                                          record_pos,
                                          LinkedList_NumElements(li),
                                          bucket_pos);
    if (record_size == kFailedWrite) {
      return kFailedWrite;
    }
    int bucket_size = WriteHTBucket(f, bucket_pos, li, fn);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int element_size = fn(f, element_pos, kv);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    position.position = static_cast<DocPositionOffset_t>((uint64_t)payload);
  ...
  int bytes = sizeof(DocIDElementHeader)
              + sizeof(DocIDElementPosition) * num_positions;

  return bytes;  // you may need to change this return value
}
