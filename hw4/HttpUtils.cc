...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // get absolute paths
  if (!realpath(root_dir.c_str(), abs_root_dir)) {
    return false;
  }
  if (!realpath(test_file.c_str(), abs_test_file)) {
    return false;
  }

  // check if root path is shorter than file path
  if (strlen(abs_root_dir) >= strlen(abs_test_file)) {
    return false;
  }

  // checks if root path appears at beginning of file path
  if (strncmp(abs_root_dir, abs_test_file, strlen(abs_root_dir)) != 0) {
    return false;
  }

  // make sure file is actually in the root directory and
  // not in a folder that starts with the same characters
  if (abs_test_file[strlen(abs_root_dir)] != '/') {
    return false;
  }
