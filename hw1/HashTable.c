...
// Searches for and returns the payload with the given key
// returns NULL if unable to find payload
// Will remove node containing the payload if remove = true
static LLPayload_t FindNode(LinkedList *chain, HTKey_t key, bool remove) {
  LLIterator *iter = LLIterator_Allocate(chain);
  LLPayload_t current;

  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, &current);
    if (((HTKeyValue_t *)current)->key == key) {

      // removes payload node if remove = true
      if (remove) {
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t *payload = (HTKeyValue_t *)
                          FindNode(chain, newkeyvalue.key, false);
  ...
  if (payload == NULL) {
    ...
    LinkedList_Push(chain, (LLPayload_t)newkv);
    table->num_elements ++;
    return false;
...
bool HashTable_Find(HashTable *table,
  ...
  // Calculate which bucket and chain we're finding from.
  bucket = HashKeyToBucketNum(table, key);
  chain = table->buckets[bucket];

  HTKeyValue_t *payload = (HTKeyValue_t *)FindNode(chain, key, false);
...
bool HashTable_Remove(HashTable *table,
  ...
  HTKeyValue_t *payload = (HTKeyValue_t *)FindNode(chain, key, true);
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // Find next non-empty linked list
  if (!LLIterator_IsValid(iter->bucket_it)) {
    ...
    // Unable to find next non-empty linked list
    iter->bucket_idx = INVALID_IDX;
    return false;
  }
  return true;  // you may need to change this return value
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  LLPayload_t kv;
  LLIterator_Get(iter->bucket_it, &kv);
  keyvalue->key = ((HTKeyValue_t *)kv)->key;
  keyvalue->value = ((HTKeyValue_t *)kv)->value;
